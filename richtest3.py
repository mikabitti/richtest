...
# Your persistent status information
    ...
    persistent_status_text = "Test"
    i = 5

    testpanel = Panel(persistent_status_text,
                      title="Current Status", border_style="blue")

    livepanel = Live(testpanel, refresh_per_second=4)
    # create_status_panel(0, 0, 0)
    livepanel.start()
    added_text = input("Add text to the panel and press Enter to continue: ")

    livepanel.update(testpanel)

    persistent_status_text += f"\n{added_text}"
    testpanel = Panel(persistent_status_text,
                      title="Current Status", border_style="blue")

    Live.update(testpanel, refresh_per_second=4)
